...
export const conditional = <T>(_if: (message: T) => boolean, _then: Dispatcher<T, any> = noop, _else: Dispatcher<T, any> = noop) => (message: T) => wrapAsyncIterableIterator((_if(message) ? _then : _else)(message))
